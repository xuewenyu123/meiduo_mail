...
class RegisterView(View):
      def post(self, request):
              ...
              # 4. 数据入库
              # 方式一
              # user = User(username=username, password=password, moblie=mobile)
              # user.save()
              # 方式二
              # User.objects.create(username=username, password=password, moblie=moblie)
              # 密码没有加密的解决方法
              user = User.objects.create_user(username=username, password=password, moblie=mobile)
